...
pub enum MpirOp {
    ...
    Move { v: MpirValue },
    BorrowShared { v: MpirValue },
    BorrowMut { v: MpirValue },
    ...
    GetField { obj: MpirValue, field: String },

    // Integer arithmetic (checked)
    IAdd { lhs: MpirValue, rhs: MpirValue },
    ISub { lhs: MpirValue, rhs: MpirValue },
    IMul { lhs: MpirValue, rhs: MpirValue },
    ISDiv { lhs: MpirValue, rhs: MpirValue },
    IUDiv { lhs: MpirValue, rhs: MpirValue },
    ISRem { lhs: MpirValue, rhs: MpirValue },
    IURem { lhs: MpirValue, rhs: MpirValue },

    // Integer arithmetic (wrapping)
    IAddWrap { lhs: MpirValue, rhs: MpirValue },
    ISubWrap { lhs: MpirValue, rhs: MpirValue },
    IMulWrap { lhs: MpirValue, rhs: MpirValue },

    // Integer arithmetic (checked -> TOption)
    IAddChecked { lhs: MpirValue, rhs: MpirValue },
    ISubChecked { lhs: MpirValue, rhs: MpirValue },
    IMulChecked { lhs: MpirValue, rhs: MpirValue },

    // Bitwise
    IAnd { lhs: MpirValue, rhs: MpirValue },
    IOr { lhs: MpirValue, rhs: MpirValue },
    IXor { lhs: MpirValue, rhs: MpirValue },
    IShl { lhs: MpirValue, rhs: MpirValue },
    ILshr { lhs: MpirValue, rhs: MpirValue },
    IAshr { lhs: MpirValue, rhs: MpirValue },
    ...
    FAdd { lhs: MpirValue, rhs: MpirValue },
    FSub { lhs: MpirValue, rhs: MpirValue },
    FMul { lhs: MpirValue, rhs: MpirValue },
    FDiv { lhs: MpirValue, rhs: MpirValue },
    FRem { lhs: MpirValue, rhs: MpirValue },

    // Float (fast-math opt-in)
    FAddFast { lhs: MpirValue, rhs: MpirValue },
    FSubFast { lhs: MpirValue, rhs: MpirValue },
    FMulFast { lhs: MpirValue, rhs: MpirValue },
    FDivFast { lhs: MpirValue, rhs: MpirValue },

    // Cast
    Cast { to: TypeId, v: MpirValue },

    // Unsafe raw pointer ops
    PtrNull { to: TypeId },
    PtrAddr { p: MpirValue },
    PtrFromAddr { to: TypeId, addr: MpirValue },
    PtrAdd { p: MpirValue, count: MpirValue },
    PtrLoad { to: TypeId, p: MpirValue },
    PtrStore {
        to: TypeId,
        p: MpirValue,
        v: MpirValue,
    ...
    Share { v: MpirValue },
    CloneShared { v: MpirValue },
    CloneWeak { v: MpirValue },
    WeakDowngrade { v: MpirValue },
    WeakUpgrade { v: MpirValue },
    ...
    EnumTag { v: MpirValue },
    EnumPayload { variant: String, v: MpirValue },
    EnumIs { variant: String, v: MpirValue },
    ...
    ArrNew { elem_ty: TypeId, cap: MpirValue },
    ArrLen { arr: MpirValue },
    ArrGet { arr: MpirValue, idx: MpirValue },
    ArrSet {
        arr: MpirValue,
        idx: MpirValue,
        val: MpirValue,
    ...
    ArrPop { arr: MpirValue },
    ArrSlice {
        arr: MpirValue,
        start: MpirValue,
        end: MpirValue,
    },
    ArrContains { arr: MpirValue, val: MpirValue },
    ArrSort { arr: MpirValue },
    ArrMap { arr: MpirValue, func: MpirValue },
    ArrFilter { arr: MpirValue, func: MpirValue },
    ArrReduce {
        arr: MpirValue,
        init: MpirValue,
        func: MpirValue,
    ...
    MapLen { map: MpirValue },
    MapGet { map: MpirValue, key: MpirValue },
    MapGetRef { map: MpirValue, key: MpirValue },
    MapSet {
        map: MpirValue,
        key: MpirValue,
        val: MpirValue,
    },
    MapDelete { map: MpirValue, key: MpirValue },
    MapContainsKey { map: MpirValue, key: MpirValue },
    MapDeleteVoid { map: MpirValue, key: MpirValue },
    MapKeys { map: MpirValue },
    MapValues { map: MpirValue },

    // String intrinsics
    StrConcat { a: MpirValue, b: MpirValue },
    StrLen { s: MpirValue },
    StrEq { a: MpirValue, b: MpirValue },
    StrSlice {
        s: MpirValue,
        start: MpirValue,
        end: MpirValue,
    },
    StrBytes { s: MpirValue },
    StrBuilderNew,
    StrBuilderAppendStr { b: MpirValue, s: MpirValue },
    StrBuilderAppendI64 { b: MpirValue, v: MpirValue },
    StrBuilderAppendI32 { b: MpirValue, v: MpirValue },
    StrBuilderAppendF64 { b: MpirValue, v: MpirValue },
    StrBuilderAppendBool { b: MpirValue, v: MpirValue },
    StrBuilderBuild { b: MpirValue },

    // String parse intrinsics
    StrParseI64 { s: MpirValue },
    StrParseU64 { s: MpirValue },
    StrParseF64 { s: MpirValue },
    StrParseBool { s: MpirValue },

    // JSON
    JsonEncode { ty: TypeId, v: MpirValue },
    JsonDecode { ty: TypeId, s: MpirValue },
...
pub fn print_mpir(module: &MpirModule, type_ctx: &TypeCtx) -> String {
    ...
    let digest_input = format!("magpie:mpir:module_digest:v0.1|{}|{}", module.path, module.sid.0);
    ...
    let mut types = module.type_table.types.clone();
    ...
    for f in fns {
        ...
        let sigdigest = blake3::hash(sigdigest_input.as_bytes()).to_hex().to_string();
        ...
        writeln!(
            out,
            "  fn @{} sid \"{}\" sigdigest \"{}\" inst_id \"I:base\" ({}) -> type_id {}",
            f.name, f.sid.0, sigdigest, params, f.ret_ty.0
        )
        .unwrap();
        writeln!(out, "  {{").unwrap();
...
pub fn verify_mpir(module: &MpirModule, type_ctx: &TypeCtx, diag: &mut DiagnosticBag) -> Result<(), ()> {
    ...
    for (gid, ty, init) in &module.globals {
        ...
        check_type_exists(init.ty, &type_id_set, diag, &format!("global @g{} init", gid.0));
    }
...
fn verify_function(
    ...
    check_type_exists(func.ret_ty, type_id_set, diag, &format!("fn '{}' return type", func.name));

    if func.blocks.is_empty() {
        emit_error(
            diag,
            "MPS0010",
            &format!("Terminator violation: fn '{}' has no basic blocks", func.name),
        );
        return;
    }
    ...
    for (param, ty) in &func.params {
        let _ = param;
        check_type_exists(*ty, type_id_set, diag, &format!("fn '{}' parameter type", func.name));
    }
    ...
    let check_value = |v: &MpirValue,
        ...
        match v {
            ...
            MpirValue::Local(lid) => {
                ...
                let effective_def = if def_blk_idx == usize::MAX { 0 } else { def_blk_idx };
...
fn check_type_exists(ty: TypeId, type_id_set: &HashSet<u32>, diag: &mut DiagnosticBag, where_: &str) {
    if !type_id_set.contains(&ty.0) {
        emit_error(
            diag,
            "MPS0004",
            &format!("TypeRef violation: type_id {} not found in type table ({})", ty.0, where_),
        );
    }
}

fn phi_type_allowed(ty: TypeId, type_map: &HashMap<u32, TypeKind>, type_ctx: &TypeCtx) -> bool {
    let kind = type_map.get(&ty.0).cloned().or_else(|| type_ctx.lookup(ty).cloned());
    match kind {
...
fn mpir_op_values(op: &MpirOp) -> Vec<MpirValue> {
    match op {
        ...
        MpirOp::CallableCapture { captures, .. } => captures.iter().map(|(_, v)| v.clone()).collect(),
...
fn format_type_kind_for_mpir(kind: &TypeKind) -> String {
    match kind {
        ...
        TypeKind::BuiltinOption { inner } => format!("builtin TOption {{ inner=type_id {} }}", inner.0),
        TypeKind::BuiltinResult { ok, err } => {
            format!("builtin TResult {{ ok=type_id {}, err=type_id {} }}", ok.0, err.0)
        }
...
fn format_heap_base(base: &HeapBase) -> String {
    match base {
        ...
        HeapBase::BuiltinArray { elem } => format!("heap_builtin Array {{ elem=type_id {} }}", elem.0),
        HeapBase::BuiltinMap { key, val } => {
            format!("heap_builtin Map {{ key=type_id {}, val=type_id {} }}", key.0, val.0)
        }
        ...
        HeapBase::BuiltinCell { inner } => format!("heap_builtin Cell {{ inner=type_id {} }}", inner.0),
        ...
        HeapBase::Callable { sig_sid } => format!("heap_builtin TCallable {{ sig=@{}, caps=[] }}", sig_sid.0),
